...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		healthBar.scale.set(0.4, 5);
		uiGroup.add(healthBar);

		var bgbar:FlxSprite = new FlxSprite(0,470).loadGraphic(Paths.image('BookBar'));
		bgbar.scrollFactor.set(1, 1);
		bgbar.setGraphicSize(Std.int(bgbar.width *2));
		bgbar.setGraphicSize(Std.int(bgbar.height * 2));
		bgbar.antialiasing = ClientPrefs.data.antialiasing;
		bgbar.screenCenter(X);
		bgbar.scrollFactor.set();
		bgbar.visible = !ClientPrefs.data.hideHud;
		bgbar.alpha = ClientPrefs.data.healthBarAlpha;
		uiGroup.add(bgbar);

		iconP1 = new HealthIcon(boyfriend.healthIcon, true);
		iconP1.scrollFactor.set(0, 0);
		iconP1.y = healthBar.y - 190;
		iconP1.x = 635;
		...
		iconP2.y = healthBar.y - 150;
		iconP2.x = 515;
		iconP2.visible = !ClientPrefs.data.hideHud;
		iconP2.alpha = ClientPrefs.data.healthBarAlpha;
		uiGroup.add(iconP2);

		var objectNames:Array<String> = ["healthBar", "bgbar", "iconP1", "iconP2"];
	...
	{
		var mult:Float = FlxMath.lerp(1, iconP1.scale.x, Math.exp(-elapsed * 0 * playbackRate));
		iconP1.scale.set(0.8, 0.8);
		iconP1.updateHitbox();

		var mult:Float = FlxMath.lerp(1, iconP2.scale.x, Math.exp(-elapsed * 0 * playbackRate));
