...
function onCreatePost()
    ...
    if stringStartsWith(version, "0.7") then
        ...
        hudItems.opaque = {"scoreTxt","timeBar","timeTxt","healthBar","iconP1","iconP2"}
        hudItems.nonOpaque = {"bar","counter","yippee"}
...
function onEvent(eventName, value1, value2)
    if eventName == "Cinematics(v3.1)" then
        ...
        if tonumber(barProperties[1]) > 0 then
            ...
            if fadeHUD then
                for i = 1, #hudItems.opaque do
                    if getProperty(hudItems.opaque[i]) ~= nil then
                        doTweenAlpha("opaqueFade" .. i, hudItems.opaque[i], 0, tonumber(barProperties[2]), "quadOut")
                    else
                        debugPrint("Warning: hudItem '" .. hudItems.opaque[i] .. "' does not exist.")
                    end
                end

                for i = 1, #hudItems.nonOpaque do
                    if getProperty(hudItems.nonOpaque[i]) ~= nil then
                        doTweenAlpha("nonOpaqueFade" .. i, hudItems.nonOpaque[i], 0, tonumber(barProperties[2]), "quadOut")
                    else
                        debugPrint("Warning: hudItem '" .. hudItems.nonOpaque[i] .. "' does not exist.")
                    end
                end
            end
